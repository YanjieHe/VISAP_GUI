...
namespace 统计图形界面1
{
    public partial class Form1 : Form
    {
        ...
        {
            string result = "\t 样本数 \t 均值 \t 标准差 \n";
            int StartRow = 0;
            int EndRow = dataGridView1.Rows.Count;
            ...
            if (Record != "AllEmpty")
            {
                ...
                for (int i = 0; i < NumberSeries.Length; i++)
                {
                    result = result + "\t"+NumberSeries[i].Length + "\t"+MathV.round(Stat.Mean(NumberSeries[i]).ToString(), 4, 0).ToString() + "\t"+MathV.round(Stat.Variance(NumberSeries[i]).Power(new BigNumber("0.5"), 30).ToString(), 4, 0) + "\n";
                    /*for (int j = 0; j < NumberSeries[i].Length; j++)
                    {
                        MessageBox.Show(NumberSeries[i][j].ToString());
                    }*/
