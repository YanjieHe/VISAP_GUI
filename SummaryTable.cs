...
namespace 统计图形界面1
{
    public partial class Form1 : Form
    {
        ...
        public string VariableNamePolish(string VarName)
        {
            ...
            if (VarName.Length > 12)
            {
                return VarName.Substring(0, 10) + "~" + VarName[VarName.Length - 1];
            }
            else
            {
                return VarName.PadLeft(12,' ');
            }
        }
        public  string Summary(int StartCol, int EndCol)
        {
            ...
            string result = "变量名".PadLeft(9, ' ') + "\t" + "样本数".PadLeft(9, ' ') + "\t" + "均值".PadLeft(10, ' ') + "\t" + "标准差".PadLeft(9, ' ') + "\t" + "最小值".PadLeft(9, ' ') + "\t" + "最大值".PadLeft(9, ' ') + "\r\n";
            int StartRow = 0;
